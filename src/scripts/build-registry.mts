...
const parseContent = (content: string) =>
  content
    ...
    .replace(/\n/g, '\\n')
    .replace(/"/g, "'")
    .replace(/@\/registry\/auth\/ui\//g, '@/components/ui/')
    .replace(/@\/registry\/auth\/components\//g, '@/components/auth/')
    .trim();
...
export const Index: Record<string, any> ={`;
  ...
  "${item.name}": {
    ...
    type: "${item.type}",
    ${item.dependencies?.length ? `dependencies: [${item.dependencies.map(dep => `"${dep}"`).join(', ')}],` : ''}
